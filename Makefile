...
amd64:
		...
		docker build -f Dockerfile.amd64 -t $(REPO)/$(IMAGE_NAME):amd64-$(IMAGE_VERSION) .
  		docker push $(REPO)/$(IMAGE_NAME):amd64-$(IMAGE_VERSION)
...
arm32v6:
		...
		docker build --build-arg ARCH=arm32v7 --no-cache -f Dockerfile.arm32v7 -t $(REPO)/$(IMAGE_NAME):arm32v6-$(IMAGE_VERSION) .
